...
const options = {
    ...
    //Important, so that the verify function can accept the req param is verify(req,payload,done)
...
module.exports = new Strategy(options, async (payload, done)=>{
    try{
        ...
        if((user && user.role === 'admin') || (user && user.role === 'moderator') ){
            ...
            // user no longer exists or role not authorized, Aythorization for specifig Role is restricted
