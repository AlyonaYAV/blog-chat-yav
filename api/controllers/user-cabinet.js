...
async function updateUserLogin(req, res, next){
  ...
  try{
    ...
    return res.status(200).json({ message: "You've chanjed your login", userName: result.login });
  }catch(e){
    ...
    comparePasswords(req.body.workPass, user.passwordHash, function(error, matchResult){
      ...
      if (!matchResult){
        ...
        createSalt(req.body.pass, async function(hash){
          //
          try{
            // Find user ID and update his password
            await User.findByIdAndUpdate({_id: req.user.id},{$set:{ passwordHash: hash }},{new:true});
