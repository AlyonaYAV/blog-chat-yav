<template>
  <div class="pets-reset-password">
    <h3>Enter new passwords</h3>
    <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="120px" class="pets-reset-password__ruleForm">
        ...
        <el-form-item>
          <el-button
            :class="!submitToggle ? 'opacity-bt1-sh' : 'opacity-bt1-hd'"
            type="primary"
            :loading="loading"
    ...
    <div v-if="linkPage" class="pets-reset-password__to-login-page">
    ...
    submitForm(formName){
      this.$refs[formName].validate( async (value)=>{
        if(value){
          ...
          this.$message({
              showClose: true,
              message: "You've entered incorrect date",
              type: 'error'
            });
        }
      });
...
<style lang="scss">
  .pets-reset-password{
    ...
    .el-form.pets-reset-password__ruleForm{
        .el-form-item{
            .el-form-item__label{
                width: 120px;
    ...
    .pets-reset-password__to-login-page{
      position: relative;
      left: 70%;
      top: -50px !important;
    }
