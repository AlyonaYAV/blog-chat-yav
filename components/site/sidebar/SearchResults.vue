...
.search-results{
  ...
  .search-results__posts{
    @include search-results-block(#e615a0, 45%);

  }
  .search-results__pages{
    @include search-results-block(#e615a0, 47%);
  }
  ...
  .search-results__clear{
    ...
    color:#f6f6f6;
    padding: .2em .3em;
    background-color: lighten($color: #c84685, $amount: 20%);
    border-radius: 5px;
    transition: background-color 1.5s;
    &:hover, &:active{
      background-color: lighten($color: #c84685, $amount: 35%);
      color:#3f3d3d;
    }
    &:disabled{
